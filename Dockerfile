...
FROM arm64v8/alpine:3.12 as initbasestage
...
RUN \
 ...
 mkdir -p /initrd && \
 LATEST=$(curl -sL \
	http://dl-cdn.alpinelinux.org/alpine/v${ALPINE_VERSION}/releases/aarch64/latest-releases.yaml \
	| yq read - [0].version) && \
 echo $LATEST && \
 curl -o \
        /rootfs.tar.gz -L \
        "http://dl-cdn.alpinelinux.org/alpine/v${ALPINE_VERSION}/releases/aarch64/alpine-minirootfs-${LATEST}-aarch64.tar.gz" && \
 tar xf \
	/rootfs.tar.gz \
	-C /initrd
